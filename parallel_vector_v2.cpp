...
void comb_vector::pushback(int val) {
    ...
    // first, check with thread-local capacity to see if we even need to load from
    // the shared vector to determine whether we need to update capacity. The vector
    // cannot shrink in capacity, so the vector's actual capacity is at least as large
    // as the thread-local copy of capacity
    if (curr_d->size >= comb_vector::info->capacity) {
...
int comb_vector::combine(th_info *info, descr *d, bool dont_need_to_return) {
    ...
    // cannot shrink in capacity, so the vector's actual capacity is at least as large
    // as the thread-local copy of capacity
    if ((d->offset + hcount) >= comb_vector::info->capacity) {
...
void comb_vector::allocate_bucket(int bucket_idx) {
  ...
  // update thread-local copy of capacity since a new bucket was allocated
  comb_vector::info->capacity += bucket_size;
}
